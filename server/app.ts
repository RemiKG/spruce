/* Spruce — the API app builder. Registers CORS + every /api route and (on a Node
   host) serves the built SPA. Split out from the listen entry (index.ts) so the
   same routes run unchanged behind a serverless function (api/[...path].ts) with
   serveStatic:false. All keys are read from the environment server-side; the
   client only ever calls relative /api/*. */
...
import { loadCatalog, catalogSize, findProduct } from './catalog';
...
/** Build the configured Fastify app (does NOT listen). `serveStatic` (default
 *  true) serves the built SPA; a serverless function passes false. */
export async function buildApp(opts: { serveStatic?: boolean } = {}): Promise<FastifyInstance> {
  ...
  // ---- static SPA (prod, Node host) + SPA fallback -------------------------
  if (serveStatic && existsSync(CLIENT_DIR)) {
    ...
    // serverless (SPA served by the platform) or no build yet — API-only 404s stay JSON.
    app.setNotFoundHandler((_req, reply) => reply.code(404).send({ error: 'not found' }));
    if (serveStatic) console.log('[spruce] no dist/client yet — run `npm run dev` (Vite serves the client and proxies /api here)');
  }
